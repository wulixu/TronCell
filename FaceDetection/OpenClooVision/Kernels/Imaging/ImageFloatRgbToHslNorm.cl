__kernel void ImageFloatRgbToHslNorm(
...
{                             
	...
	if (max != min) 
	{
		...
		if (max == color.x) hsl.x = (color.y - color.z) / (max - min); else
			if (max == color.y) hsl.x = 2.0f + (color.z - color.x) / (max - min); else
				if (max == color.z) hsl.x = 4.0f + (color.x - color.y) / (max - min);
